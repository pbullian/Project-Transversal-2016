...
{
	//char com1[strlen(com)];
	
	char* Delim=" ";
	int i=0,j=0;
		
	/*for (j=0;j<strlen(com)+1;j++)
		com1[j]=com[j];
		
	Res[i]=strtok(com, Delim);
	i++;
	while(Res[i]!=NULL){
		strcpy(Res[i], strtok(NULL, Delim));//On récupère les différentes parties du message(séparés par une espace)
		i++;
	}
	//putchar('0' + i);
