...
// Flag pour la lecture de inbuf 
unsigned char flaginbuf=0;
unsigned char flagCR=0;

//**************************************************************************************************
//**************************************************************************************************
void UART1_ISR(void) interrupt 0x4 {
...
unsigned char serOutstring(char *buf) {
	//char *buf1=buf;
unsigned char len;
unsigned char	xdata code_err=0;
...
// *************************************************************************************************
// serInBuf:  Récupération d'une chaine de caractères dans le buffer de réception
// ************************************************************************************************
char* serInBuf(void) {
	if (!RB_EMPTY(&in))
  {   
		int i=0,j=0;
		//static RB_CREATE(copy_in, unsigned char xdata);
		char  xdata s[MAX_BUFLEN+1];
		while((&in)->rb_in != (&in)->rb_out){
			s[i]=*RB_POPSLOT(&in);  
			RB_POPADVANCE(&in);
			i++;
		}
